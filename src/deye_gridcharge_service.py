...
class DeyeGridChargeService(DeyeEventProcessor):
    def __init__(
        self, logger_config: DeyeLoggerConfig, mqtt_client: DeyeMqttClient, sensors: list[Sensor], modbus: DeyeModbus
    ):
        self.__log = logger_config.logger_adapter(logging.getLogger(DeyeGridChargeService.__name__))
        self.__logger_config = logger_config
        self.__mqtt_client = mqtt_client
        self.__sensors = [sensor for sensor in sensors if sensor.mqtt_topic_suffix.startswith("gridcharge")]
    ...
    def get_description(self):
        return "Grid charge configuration over MQTT"
    ...
    def initialize(self):
        ...
        self.__mqtt_client.subscribe_command_handler(
            self.__logger_config.index, "gridcharge/control", self.handle_control_command
        )
    ...
    def handle_control_command(self, client: Client, userdata, msg: MQTTMessage):
        if msg.payload == b"write":
            ...
            self.__log.info("GridCharge modifications cleared")

    def write_config(self, dry_run: bool):
        if not self.__read_state:
            self.__log.warning("Grid charge state not yet read from the inverter. Cannot apply config modifications.")
            return
    ...
    def __write_registers(self, reg_map: dict[int, bytearray], dry_run: bool) -> None:
        ...
        for reg_addr in range(
            ...
            if reg_addr in reg_map:
                ...
                self.__log.info(f"Write grid charge config registers: {batch_reg_addr}: {reg_data}")
                if not dry_run:
